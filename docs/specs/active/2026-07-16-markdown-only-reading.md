...
```ts
interface RenderedTextPosition {
  blockId: string;
  offsetInBlock: number;
}
```

渲染文本选区保存起止位置、选中文字及派生 breadcrumb。可选择的 block 在 DOM 上暴露稳定 `data-block-id`；跨 block 选区由起止 `RenderedTextPosition` 表达。定位优先使用 block ID，文字高亮使用 `offsetInBlock`。
