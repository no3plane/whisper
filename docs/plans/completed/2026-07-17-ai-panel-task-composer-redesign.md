...
- [x] **Step 1: 写出补充提问可空、解读方式必选的失败测试**

```ts
it('目标有效且已选解读方式时允许空补充提问', () => {
  expect(validateDraft({ ...draft, prompt: '', skillType: 'book_summary' })).toEqual({
    valid: true,
  });
});
...
export type DraftValidation =
  { valid: true } | { valid: false; reason: 'method-required' | 'method-not-allowed' };
...
- [x] **Step 4: 写出当前阅读位置和选区派生可选目标的失败测试**

```ts
expect(
  buildTargetOptions(chapters, 'section-1', selectionTarget).map((target) => target.type),
).toEqual(['book', 'chapter', 'chapter', 'selection']);
...
- [x] **Step 4: 把 TargetPicker 收敛为单行摘要与按需菜单**
...
- [x] **Step 5: 重写 DraftComposer 为紧凑三行任务**
...
- [x] **Step 6: 实现约 360px 宽下的紧凑 CSS 并测试**
...
- [x] **Step 2: 写出 workspace 不再维护待发送引用的失败测试**
...
- [x] **Step 3: 运行测试并确认旧引用 Composer 导致失败**
...
- [x] **Step 5: 在 ReaderPage 中持久化并更新本地快照**
...
- [x] **Step 2: 运行相关 renderer 与 shared 测试**
