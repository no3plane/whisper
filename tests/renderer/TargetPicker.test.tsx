...
const selectionTarget: ReadingTarget = {
  ...
  selectedText: '所谓自由', startOffset: 0, endOffset: 4,
  breadcrumb: [
...
const draft: ConversationDraft = {
  bookId: 'book-1', target: selectionTarget, contextStrategy: 'hybrid', mode: 'auto',
  strategySource: 'book-default', skillType: 'plain_explanation', prompt: '', reference: null,
};
...
describe('SelectionMenu', () => {
  ...
  it('正式态提供新会话和引用两个入口', () => {
    ...
    render(<SelectionMenu mode="thread" selectedText="所谓自由" onSetTarget={vi.fn()} onStartConversation={onStartConversation} onReference={onReference} />);
...
describe('TargetPicker', () => {
  it('点击父章节更新目标且技能只呈现当前目标可用项', () => {
    ...
    render(<TargetPicker draft={draft} onTargetChange={onTargetChange} onSkillChange={vi.fn()} onStrategyChange={vi.fn()} />);
    fireEvent.click(screen.getByRole('button', { name: '第八章' }));
    expect(onTargetChange).toHaveBeenCalledWith(expect.objectContaining({ type: 'chapter', chapterId: 'chapter' }));
    expect(screen.getAllByRole('button', { name: '白话解释' })).toHaveLength(1);
  });

  it('父状态清除技能时显示轻提示', () => {
    const { rerender } = render(<TargetPicker draft={draft} onTargetChange={vi.fn()} onSkillChange={vi.fn()} onStrategyChange={vi.fn()} />);
    rerender(<TargetPicker draft={{ ...draft, skillType: null }} onTargetChange={vi.fn()} onSkillChange={vi.fn()} onStrategyChange={vi.fn()} />);
