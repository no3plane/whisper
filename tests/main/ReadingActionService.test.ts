...
const target: ReadingTarget = {
  type: 'selection', chapterId: 'c1', startPassageId: 'p1', endPassageId: 'p1',
  selectedText: '原文', startOffset: 0, endOffset: 2,
  breadcrumb: [{ chapterId: 'c1', title: '第一章' }],
};
const document = {
  book: { id: 'book-1', title: '测试书' },
  chapters: [{ id: 'c1', bookId: 'book-1', parentChapterId: null, title: '第一章', level: 1, order: 0, startPassageId: 'p1', endPassageId: 'p1', summary: null }],
  passages: [{ id: 'p1', bookId: 'book-1', chapterId: 'c1', order: 0, text: '原文', sourceHref: null, sourceOffset: 0 }],
  fullText: '原文',
};
...
function setup(providerResult: 'success' | 'failure' = 'success') {
  ...
  const store = {
    ...
    deleteThread(id: string) { const index = threads.findIndex((thread) => thread.id === id); if (index < 0) throw new Error('missing'); threads.splice(index, 1); },
  };
  const provider = {
    async streamGenerate(_settings: any, _context: any, handlers: any) {
      if (providerResult === 'failure') throw new Error('网络错误');
      handlers.onChunk('回答'); return { text: '回答', usage: 3 };
    },
  };
  const window = { isDestroyed: () => false, webContents: { send() {} } } as any;
  const service = new ReadingActionService(
    { getAISettings: () => ({ baseURL: '', apiKey: '', model: 'test', contextWindow: 10000, defaultContextStrategy: 'full_book' }) } as any,
...
describe('ReadingActionService', () => {
  it('没有技能时拒绝空白首次问题', async () => {
    ...
    await expect(service.createConversation({ bookId: 'book-1', target, skillType: null, prompt: '  ', contextStrategy: 'full_book' }, window)).rejects.toThrow('请输入问题');
  });
  ...
  it('引用追问把 reference 写入 user message', async () => {
    const { service, messages, window } = setup();
    const created = await service.createConversation({ bookId: 'book-1', target, skillType: 'plain_explanation', prompt: '', contextStrategy: 'full_book' }, window);
    const reference = { selectedText: '引用', startPassageId: 'p1', endPassageId: 'p1', startOffset: 0, endOffset: 2, breadcrumb: [] };
    await service.followUp({ threadId: created.thread.id, question: '这是什么意思？', reference }, window);
  ...
  it('流式失败会持久化 assistant message 的失败状态', async () => {
    ...
    await expect(service.createConversation({ bookId: 'book-1', target, skillType: 'plain_explanation', prompt: '', contextStrategy: 'full_book' }, window)).rejects.toThrow('网络错误');
  ...
  it('retry 复用失败 assistant message ID 且不新增 message', async () => {
    ...
    const success = { async streamGenerate(_s: any, _c: any, handlers: any) { handlers.onChunk('重试'); return { text: '重试', usage: 2 }; } };
    const service = new ReadingActionService((failed.service as any).settings, (failed.service as any).library, failed.store as any, success as any);
