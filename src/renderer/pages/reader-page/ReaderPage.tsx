...
export function ReaderPage({ bookId, onBack }: ReaderPageProps) {
    ...
    void (async () => {
      try {
        ...
        const defaultThreadId =
          history.activeThreadId && known.has(history.activeThreadId)
        ...
        const initial =
          history.activeThreadId && stored.includes(history.activeThreadId)
            ? history.activeThreadId
            : stored[0];
  ...
  useEffect(() => {
    if (document) localStorage.setItem(openThreadsKey(bookId), JSON.stringify(openThreadIds));
  }, [bookId, document, openThreadIds]);
  useEffect(() => {
    ...
    () =>
      whisper.ai.onStream((event) => {
        ...
        setOpenThreadIds((ids) =>
          ids.includes(event.thread.id) ? ids : [...ids, event.thread.id],
        );
  ...
  function clearSourceHighlight() {
    ...
    highlightedPassage.current?.classList.remove(styles.temporarySourceHighlight);
  ...
  async function createConversation(input: CreateConversationInput) {
    try {
      ...
      setOpenThreadIds((ids) =>
        ids.includes(result.thread.id) ? ids : [...ids, result.thread.id],
      );
  ...
  function locate(threadId: string, reference?: MessageReference | null) {
    ...
    const passage = snapshot.startPassageId
      ? ([...articleRef.current.querySelectorAll<HTMLElement>('[data-passage-id]')].find(
    ...
    const anchor = exact
      ? (range!.startContainer.parentElement?.closest<HTMLElement>('[data-passage-id]') ?? passage)
      : passage;
    if (!anchor) {
      setNotice('无法恢复原文位置。');
      return;
    ...
    if (exact && browserSelection) {
      ...
      anchor.classList.add(styles.temporarySourceHighlight);
      highlightedPassage.current = anchor;
      setNotice('无法恢复精确选区，已定位到相关段落。');
    }
  ...
  const activeThread =
    activeView?.type === 'thread'
      ? threads.find((item) => item.thread.id === activeView.threadId)
      : null;
  const streamError =
    activeThread?.messages.find((message) => message.status === 'failed')?.error ??
    activeThread?.thread.lastError ??
    undefined;
  return (
    <section className={styles.layout}>
      ...
      <RightAiPanel
        threads={threads}
        historyThreads={threads.map(({ thread }) => thread)}
        ...
        onRetryThread={(id) => {
          const failed = threads
            .find((item) => item.thread.id === id)
            ?.messages.find(
              (message) => message.role === 'assistant' && message.status === 'failed',
            );
          if (failed) void retryMessage(id, failed.id);
        }}
        onFollowUp={followUp}
        onClearReference={() => setPendingReference(null)}
        onRetryMessage={(id, messageId) => void retryMessage(id, messageId)}
        onLocate={locate}
        retryableThreadIds={
          new Set(
            threads
...
function updateFromStream(
  ...
  if (event.type === 'started' || event.type === 'done') {
    setThreads((items) => upsertThread(items, event.thread, event.messages));
    return;
  }
  setThreads((items) =>
    items.map((item) =>
...
function upsertThread(items: ThreadItem[], thread: ReadingThread, messages: ThreadMessage[]) {
  const next = { thread, messages };
  return items.some((item) => item.thread.id === thread.id)
    ? items.map((item) => (item.thread.id === thread.id ? next : item))
    : [...items, next];
}
