...
function entries(buffer: Buffer): Map<string, Buffer> {
  ...
  if (offset < 0) {
    throw new Error('EPUB 不是有效的 ZIP 文件：缺少 central directory。');
  }
  const count = buffer.readUInt16LE(offset + 10);
  offset = buffer.readUInt32LE(offset + 16);
  for (let i = 0; i < count; i += 1) {
    if (buffer.readUInt32LE(offset) !== 0x02014b50) {
      throw new Error('EPUB central directory 损坏。');
    }
    ...
    if (!name.endsWith('/')) {
      if (method === 0) {
        output.set(name, compressed);
      } else if (method === 8) {
        output.set(name, inflateRawSync(compressed));
      } else {
        throw new Error(`EPUB 使用了不支持的压缩算法：${method}`);
      }
    }
...
export class EpubParser {
  parse(input: { bookId: string; buffer: Buffer }) {
    ...
    if (!opfPath) {
      throw new Error('EPUB 缺少 container.xml 或 OPF rootfile。');
    }
    const opf = files.get(opfPath)?.toString('utf8');
    if (!opf) {
      throw new Error(`EPUB 无法读取 manifest：${opfPath}`);
    }
    ...
    const spine = [...opf.matchAll(/<itemref\b[^>]*\bidref=["']([^"']+)["'][^>]*>/gi)].map(
      (match) => match[1],
    );
    if (manifest.size === 0) {
      throw new Error('EPUB manifest 为空。');
    }
    if (spine.length === 0) {
      throw new Error('EPUB spine 为空。');
    }
    ...
    for (const [chapterOrder, idref] of spine.entries()) {
      ...
      if (!href) {
        throw new Error(`EPUB spine 引用了不存在的 manifest item：${idref}`);
      }
      const filePath = path.posix.normalize(
        path.posix.join(base, decodeURIComponent(href.split('#')[0])),
      );
      const html = files.get(filePath)?.toString('utf8');
      if (!html) {
        throw new Error(`EPUB 章节文件不可读：${filePath}`);
      }
      const title = decode(
      ...
      for (const text of blocks) {
        passages.push({
          ...
          text,
          sourceHref: href,
          sourceOffset: html.indexOf(text),
        });
      }
      if (passages.length > start) {
        chapters.push({
          id: chapterId,
          bookId: input.bookId,
          ...
          level: 1,
          order: chapterOrder,
          startPassageId: passages[start].id,
          endPassageId: passages[passages.length - 1].id,
          summary: null,
        });
      }
    }
